...
namespace KeeShare
{
    public class SyncExporter
    {
        ...
        {
            PwDatabase copyDB = new PwDatabase();
            copyDB.RootGroup = new PwGroup(true, true)
            {
            ...
            foreach (PwGroup group in sharedFolders)
            {
                foreach (PwEntry entry in group.Entries)
                {
                    ...
                    if (entry.IsNormalPwEntry())
                    {
                        ...
                        if (null != copyDB.RootGroup.FindEntry(entry.Uuid, true))
                        {
                            continue;
                        }
                        ...
                        {
                            entry.SetParent(copyDB.RootGroup);
                            copyEntry = entry;
                        }
                        else  //the entry was located in a normal PwGroup
                        {
                            copyEntry = database.DuplicateEntryTo(entry, copyDB);
                        }
                    }
                    //or a pwproxy
                    if (database.IsPasswordProxy( entry ))
                    {
                        ...
                        //we dont want to share a PwEntry more than once!
                        if (null != copyDB.RootGroup.FindEntry(entryRoot.Uuid, true))
                        {
                            continue;
                        }
                        copyEntry = database.DuplicateEntryTo(entryRoot, copyDB);
                    }
                    if (copyEntry != null )
                    {
        ...
        private void ExportDatabaseTo(PwDatabase deltaDB, PwEntry userRoot, List<string> paths)
        {
            ...
            deltaDB.Register(KeeShare.DeltaDatabaseId, KeeShare.DeltaDatabaseTag);
